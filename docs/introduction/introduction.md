...
- **One branch, one configuration.** The `master`, `ancient_dna` and `custom_alleles` branches have been merged. What used to be a branch choice is now a key in `config.yaml`: `read_mode` selects short, ancient or long reads, and `allele_source` selects assemblies or a curated allele panel.
...
- **Reworked benchmarking**, reporting QV and error rate per haplotype, in either a leave-zero-out or a leave-all-out design.
...
Bolognini, D. et al. (2026). Population-scalable genotyping from low-coverage sequencing data using pangenome graphs. **bioRxiv** 2026.02.05.704023
